...
Cards show the profile, type, title, time, labels, and a short excerpt of existing health details; no clinical conclusions are generated. An event opens its details, and a document opens the related event's attachments section. Shared profile/type/category/tag filters and search apply; the local Year field narrows the date range, while Show can restrict the timeline to events or documents. A year outside a shared date range displays an actionable validation message. The document search follows the parent event's searchable health fields.
...
Choose **Saved doctor** in an event form to link a provider. Existing free-text doctor entries remain supported and are not automatically matched by name. Selecting a saved doctor also records its current name in the event's doctor text. A provider rename updates its linked display name; deleting the provider clears the reference and retains the event's recorded text. The provider's detail page shows contact information and provider notes, followed by related appointments, diagnoses, prescriptions, tests, and event notes. Records are paginated 30 events at a time across both profiles; each section explicitly reflects that page, and all linked event types remain accessible in **All linked events**.

The **Doctor** filter under More filters applies to Timeline (including an event's uploaded documents), Events, Calendar, and Dashboard. It matches the saved `provider_id`, not free-text doctor names. Shared filters are unchanged unless you explicitly select a doctor.
