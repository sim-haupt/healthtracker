...
alter table public.health_events
  add column severity text check (severity in ('Mild','Moderate','Severe'));
...
begin
  ...
  if p_id is null then
    insert into public.health_events(owner_id,profile_id,event_type,title,description,event_date,end_date,symptoms,diagnosis,treatment,prescription,doctor,location,notes,category_id,provider_id,test_type,severity,disease,dose_number,dose_total,next_dose_date,needs_renewal,renewal_date)
    values (auth.uid(),v.profile_id,v.event_type,v.title,coalesce(v.description,''),v.event_date,v.end_date,v.symptoms,v.diagnosis,v.treatment,v.prescription,v.doctor,v.location,v.notes,v.category_id,v.provider_id,v.test_type,v.severity,v.disease,v.dose_number,v.dose_total,v.next_dose_date,coalesce(v.needs_renewal,false),v.renewal_date)
    ...
    update public.health_events set profile_id=v.profile_id,event_type=v.event_type,title=v.title,description=coalesce(v.description,''),
      ...
      test_type=v.test_type,severity=v.severity,disease=v.disease,dose_number=v.dose_number,dose_total=v.dose_total,next_dose_date=v.next_dose_date,
...
language sql stable security invoker set search_path = '' as $$
  select jsonb_build_object('id',p_event.id,'profile_id',p_event.profile_id,'event_type',p_event.event_type,
    ...
    'event_date',p_event.event_date,'end_date',p_event.end_date,'category_id',p_event.category_id,'test_type',p_event.test_type,'severity',p_event.severity,'disease',p_event.disease,
...
language sql stable security invoker set search_path = '' as $$
  ...
  where (nullif(p_filters->>'profile_id','') is null or e.profile_id=(p_filters->>'profile_id')::uuid)
    ...
    and (nullif(btrim(p_filters->>'q'),'') is null or strpos(lower(concat_ws(' ',e.title,e.test_type,e.severity,e.disease,e.description,e.symptoms,e.diagnosis,e.notes)),lower(btrim(p_filters->>'q')))>0);
$$;
