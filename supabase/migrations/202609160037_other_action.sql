...
alter table public.health_events
  add column action text check (length(action) <= 5000);
...
language sql stable security invoker set search_path = '' as $$
  ...
  where (nullif(p_filters->>'profile_id','') is null or e.profile_id=(p_filters->>'profile_id')::uuid)
    ...
    and (nullif(btrim(p_filters->>'q'),'') is null or strpos(lower(concat_ws(' ',e.title,e.test_type,e.severity,e.trigger,e.relief,e.injury_type,e.body_area,e.frequency,e.recovery,e.action,e.disease,e.description,e.symptoms,e.diagnosis,e.notes)),lower(btrim(p_filters->>'q')))>0);
$$;
