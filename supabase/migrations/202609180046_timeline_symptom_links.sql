...
returns jsonb language sql stable security invoker set search_path='' as $$
 with matching as materialized (
   ...
   select 'event:'||e.id::text as id,'event'::text as entry_type,e.id as event_id,e.profile_id,e.event_type,
     ...
     (select r.source_event_id from public.health_event_relations r
       where r.target_event_id=e.id and r.relation_type='visit_symptom' order by r.created_at desc,r.source_event_id limit 1) as related_event_id,
   ...
   select 'episode:'||ep.id::text,'episode',ep.id,ep.profile_id,'Health Episode',ep.title,ep.title,
     ...
     '[]'::jsonb,null::jsonb,null::uuid,null::text,null::bigint
