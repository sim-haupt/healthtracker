...
alter table public.health_events
  add column injury_type text check (length(injury_type) <= 300),
  add column body_area text check (length(body_area) <= 300),
  add column recovery text check (length(recovery) <= 5000);
...
begin
  ...
  if p_id is null then
    ...
    update public.health_events set profile_id=v.profile_id,event_type=v.event_type,title=v.title,description=coalesce(v.description,''),
      ...
      test_type=v.test_type,severity=v.severity,trigger=v.trigger,relief=v.relief,injury_type=v.injury_type,body_area=v.body_area,recovery=v.recovery,disease=v.disease,dose_number=v.dose_number,dose_total=v.dose_total,next_dose_date=v.next_dose_date,
...
language sql stable security invoker set search_path = '' as $$
  select jsonb_build_object('id',p_event.id,'profile_id',p_event.profile_id,'event_type',p_event.event_type,
    ...
    'event_date',p_event.event_date,'end_date',p_event.end_date,'category_id',p_event.category_id,'test_type',p_event.test_type,'severity',p_event.severity,'trigger',p_event.trigger,'relief',p_event.relief,'injury_type',p_event.injury_type,'body_area',p_event.body_area,'recovery',p_event.recovery,'disease',p_event.disease,
...
language sql stable security invoker set search_path = '' as $$
  ...
  where (nullif(p_filters->>'profile_id','') is null or e.profile_id=(p_filters->>'profile_id')::uuid)
    ...
    and (nullif(btrim(p_filters->>'q'),'') is null or strpos(lower(concat_ws(' ',e.title,e.test_type,e.severity,e.trigger,e.relief,e.injury_type,e.body_area,e.recovery,e.disease,e.description,e.symptoms,e.diagnosis,e.notes)),lower(btrim(p_filters->>'q')))>0);
$$;
